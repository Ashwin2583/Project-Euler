...
# loop for traversing the matrix and finding the greatest product
for i in range(rows):
    for j in range(col):
        # right
        if j+3 < col:
        ...
        # Diagonal Right-Down
        if i + 3 < rows and j + 3 < col:
            max_prod = max(max_prod, grid[i, j] * grid[i+1, j+1] * grid[i+2, j+2] * grid[i+3, j+3])
        # Diagonal Left-Down
        if i + 3 < rows and j - 3 >= 0:
            max_prod = max(max_prod, grid[i, j] * grid[i+1, j-1] * grid[i+2, j-2] * grid[i+3, j-3])
        # Diagonal Right-Up
        if i - 3 >= 0 and j + 3 < col:
            max_prod = max(max_prod, grid[i, j] * grid[i-1, j+1] * grid[i-2, j+2] * grid[i-3, j+3])
        # Diagonal Left-Up
        if i - 3 >= 0 and j - 3 >= 0:
            max_prod = max(max_prod, grid[i, j] * grid[i-1, j-1] * grid[i-2, j-2] * grid[i-3, j-3])

print("The greatest product of four adjacent numbers in the same direction (up,down,left,right or diagonally) in the 20x20 grid: ", max_prod)